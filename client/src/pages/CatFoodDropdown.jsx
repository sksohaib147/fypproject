...
const CatFoodDropdown = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      try {
        ...
        console.log('Fetching Cat Food listings with cache buster:', cacheBuster);
        const data = await api.get(`/products?category=Cats&cb=${cacheBuster}`);
        setListings(Array.isArray(data.data) ? data.data : []);
      } catch (err) {
        console.error('Error fetching Cat Food listings:', err);
        setListings([]);
      }
      setLoading(false);
    };
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      const name = formData.title || formData.name || 'Cat Food';
      const payload = {
        ...formData,
        pricePKR,
        name,
      ...
      const data = await api.get('/products?category=Cats');
      setListings(Array.isArray(data.data) ? data.data : []);
    } catch (err) {
      ...
      <Box sx={{ maxWidth: 1200, mx: 'auto', px: 2, color: 'text.primary' }}>
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
            ...
            <Box component="ul" sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              {categories.map((cat) => (
                <Box component="li" key={cat.name}>
                  {cat.name === 'Cat Food' && (
                    <Link to="/cat-food" style={{ textDecoration: 'none', color: 'inherit' }}>
                      <Box sx={{ width: '100%', display: 'block', textAlign: 'left', p: 2, borderRadius: 2, border: 1, borderColor: 'error.200', bgcolor: 'error.50', color: 'error.main', transition: 'all 0.2s', '&:hover': { borderColor: 'error.200', boxShadow: 2 } }}>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Typography sx={{ fontWeight: 500, color: 'error.main' }}>{t.catFood}</Typography>
                        </Box>
                      </Box>
                    </Link>
                  )}
                  {cat.name !== 'Cat Food' && (
                    <Link to={cat.name === 'Dog Food' ? '/dog-food' : cat.name === 'Rabbit Food' ? '/rabbit-food' : cat.name === 'Toys' ? '/toys' : '/belts-and-cages'} style={{ textDecoration: 'none', color: 'inherit' }}>
          ...
          <Box component="main" sx={{ flex: 1 }}>
            ...
            <Typography variant="h4" fontWeight={700} mb={1} color="text.primary">Cat Food Products</Typography>
            <Typography color="text.secondary" mb={3} sx={{ maxWidth: 600 }}>
              Discover our wide selection of premium cat food for all breeds and life stages. From grain-free and high-protein formulas to wet and dry options, we have the perfect meal for your feline friend's health and happiness.
            </Typography>
