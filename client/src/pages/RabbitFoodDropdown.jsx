...
const RabbitFoodDropdown = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      setLoading(true);
      try {
        const data = await api.get('/products?category=Rabbit Food');
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      const name = formData.title || formData.name || 'Rabbit Food';
      const payload = {
        ...formData,
        pricePKR,
        name,
        description: formData.description || formData.title || formData.name || 'Rabbit Food',
      ...
      <Box sx={{ maxWidth: 1200, mx: 'auto', px: 2, color: 'text.primary' }}>
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
              ...
              {categories.map((cat) => (
                <Box component="li" key={cat.name}>
                  {cat.name === 'Rabbit Food' && (
                    <Link to="/rabbit-food" style={{ textDecoration: 'none', color: 'inherit' }}>
                      <Box sx={{ width: '100%', display: 'block', textAlign: 'left', p: 2, borderRadius: 2, border: 1, borderColor: 'error.200', bgcolor: 'error.50', color: 'error.main', transition: 'all 0.2s', '&:hover': { borderColor: 'error.200', boxShadow: 2 } }}>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Typography sx={{ fontWeight: 500, color: 'error.main' }}>{t.rabbitFood}</Typography>
                        </Box>
                      </Box>
                    </Link>
                  )}
                  {cat.name !== 'Rabbit Food' && (
                    <Link to={cat.name === 'Dog Food' ? '/dog-food' : cat.name === 'Cat Food' ? '/cat-food' : cat.name === 'Toys' ? '/toys' : '/belts-and-cages'} style={{ textDecoration: 'none', color: 'inherit' }}>
          ...
          <Box component="main" sx={{ flex: 1 }}>
            ...
            <Typography variant="h4" fontWeight={700} mb={1} color="text.primary">Rabbit Food Products</Typography>
            <Typography color="text.secondary" mb={3} sx={{ maxWidth: 600 }}>
              Discover our wide selection of premium rabbit food for all breeds and life stages. From timothy hay and high-fiber pellets to vitamin-rich blends, we have the perfect meal for your bunny's health and happiness.
            </Typography>
