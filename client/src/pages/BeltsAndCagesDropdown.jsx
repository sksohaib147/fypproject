...
const BeltsAndCagesDropdown = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      setLoading(true);
      try {
        const data = await api.get('/products?category=Belts and Cages');
  ...
  const handleAddToCart = (listing) => {
    const product = { ...listing, _id: listing.id };
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      const name = formData.title || formData.name || 'Belts and Cages';
      const payload = {
        ...formData,
        pricePKR,
        name,
        description: formData.description || formData.title || formData.name || 'Belts and Cages',
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', py: 3 }}>
      <Box sx={{ maxWidth: 1200, mx: 'auto', px: 2, color: 'text.primary' }}>
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
              ...
              {categories.map((cat) => (
                <Box component="li" key={cat.name}>
                  {cat.name === 'Belts and Cages' && (
                    <Link to="/belts-and-cages" style={{ textDecoration: 'none', color: 'inherit' }}>
                      <Box sx={{ width: '100%', display: 'block', textAlign: 'left', p: 2, borderRadius: 2, border: 1, borderColor: 'error.200', bgcolor: 'error.50', color: 'error.main', transition: 'all 0.2s', '&:hover': { borderColor: 'error.200', boxShadow: 2 } }}>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Typography sx={{ fontWeight: 500, color: 'error.main' }}>{t.beltsAndCages}</Typography>
                        </Box>
                      </Box>
                    </Link>
                  )}
                  {cat.name !== 'Belts and Cages' && (
                    <Link to={cat.name === 'Dog Food' ? '/dog-food' : cat.name === 'Cat Food' ? '/cat-food' : cat.name === 'Rabbit Food' ? '/rabbit-food' : '/toys'} style={{ textDecoration: 'none', color: 'inherit' }}>
          ...
          <Box component="main" sx={{ flex: 1 }}>
            ...
            <Typography variant="h4" fontWeight={700} mb={1} color="text.primary">Belts, Harnesses, Collars, Cages & Carriers</Typography>
            <Typography color="text.secondary" mb={3} sx={{ maxWidth: 600 }}>
              Discover our wide selection of safe, comfortable, and durable accessories for your pets. From harnesses and collars to cages and carriers, we have everything your pet needs for safety and comfort.
            </Typography>
      ...
      {isAdmin && (
        <MarketplaceListingForm
          ...
          defaultCategory="Belts and Cages"
        />
      )}
